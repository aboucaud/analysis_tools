...
class TestVectorActions(unittest.TestCase):
    ...
    def testExponentiateFromBase(self):
        power = np.arange(1, 6)
        for base in (-2.3, 0.6):
            truth = base**power
            self._testMath(ExponentiateFromBaseVector, truth, compare_exact=False, base=base, num_vectors=1)

    def testExponentiateToPower(self):
        base = np.arange(1, 6)
        for power in (-2.3, 0.6):
            truth = base**power
            self._testMath(ExponentiateToPowerVector, truth, compare_exact=False, power=power, num_vectors=1)
